...
```bash
python3 scripts/extract_sitrep.py pdfs/MVE_SitRep_NNN_YYYY-MM-DD.pdf
```
...
Outputs written to: outputs/sitreps/
```
...
| File | Description |
|------|-------------|
| `outputs/sitreps/<name>/new_cases_counts.csv` | New cases table from this sitrep |
| `outputs/sitreps/<name>/cumulative_counts.csv` | Cumulative cases table |
| `outputs/sitreps/<name>/combined_counts.csv` | Merged new + cumulative |
| `outputs/sitreps/<name>/response_counts.csv` | Patient movement summary (if present) |
| `outputs/sitreps/<name>/poe_counts.csv` | Points d'Entrée summary (if present) |
| `outputs/sitreps/<name>/raw_extraction.json` | Raw Claude JSON response (debug) |
| `outputs/master_combined_counts.csv` | Master linelist — all sitreps combined |
| `outputs/master_response_counts.csv` | Master patient movement data |
| `outputs/master_poe_counts.csv` | Master Points d'Entrée data |
| `outputs/processed.json` | Registry of extracted PDFs (keys are canonical PDF filenames) |
...
| `ValueError: Failed to extract valid JSON after repair` | Claude returned unparseable output for a PDF | Inspect `outputs/sitreps/<name>/raw_extraction.json`; re-run single-PDF extraction: `python3 scripts/extract_sitrep.py pdfs/<name>.pdf` |
...
| `FileNotFoundError: pdfs/MVE_SitRep_NNN...pdf` | PDF not in archive | Run `sitrep-fetch` first; verify with `ls pdfs/*.pdf` |
...
| Master CSV has duplicate rows | Single-PDF mode run after update mode | Deduplicate: `python3 -c "import pandas as pd; df=pd.read_csv('outputs/master_combined_counts.csv'); df.drop_duplicates().to_csv('outputs/master_combined_counts.csv', index=False)"` |
...
- `outputs/processed.json` keys are canonical PDF filenames (e.g., `MVE_SitRep_002_2026-05-18.pdf`)
- The extraction folder for SitRep 002 is `outputs/sitreps/MVE_SitRep_002_2026-05-20/` (date from original download's Last-Modified header) — this is correct and expected
