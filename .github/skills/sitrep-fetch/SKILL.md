...
- Rebuilding or inspecting `pdfs/manifest.json`
...
|------|---------|---------|
| `--pdf-dir DIR` | `pdfs/` | Local PDF archive directory |
...
|--------|---------|-----------|
| `N new PDF(s) downloaded → pdfs/` | New sitreps available | Proceed to `sitrep-extract` |
...
| `json.JSONDecodeError` reading manifest | `pdfs/manifest.json` corrupted | Delete it with `rm pdfs/manifest.json` and re-run fetch — the script rebuilds the manifest from the existing PDFs in `pdfs/` |
...
| File | Description |
|------|-------------|
| `pdfs/manifest.json` | Registry of all known URLs → canonical PDF names; includes `duplicate_of` for deduplicated entries |
| `pdfs/MVE_SitRep_NNN_YYYY-MM-DD.pdf` | Canonical PDF archive (9 files as of 2026-05-27) |
| `outputs/processed.json` | Extraction registry (updated by `sitrep-extract`, not this step) |

## Notes

- `pdfs/` is in `.gitignore` — `pdfs/manifest.json` is **not** git-tracked
